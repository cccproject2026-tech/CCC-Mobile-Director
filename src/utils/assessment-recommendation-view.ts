...
export type CdpSectionRow = RecommendationSectionView & {
...
function recommendationValueToLines(value: unknown): string[] {
  ...
  const direct =
    row.message ?? row.text ?? row.description ?? row.label ?? row.cdp ?? row.mentorCdp;
...
/** Web: parseRecommendationSectionsForPastorView */
...
/** Web: collectSentSectionIds — only show sections marked sent when API provides flags. */
export function collectSentSectionIds(payload: unknown): Set<string> {
...
function extractAnswersScoreBySection(answersPayload: unknown): Record<string, number> {
  const scores: Record<string, number> = {};
  if (!answersPayload || typeof answersPayload !== 'object') return scores;

  const root = answersPayload as Record<string, unknown>;
  const inner = (root.data as Record<string, unknown>) ?? root;
  const sections = inner.sections;

  if (!Array.isArray(sections)) return scores;

  sections.forEach((section: unknown) => {
    if (!section || typeof section !== 'object') return;
    const row = section as Record<string, unknown>;
    const sectionId = String(row.sectionId ?? row._id ?? '').trim();
    const score = row.sectionScore ?? row.score;
    if (sectionId && score != null && !Number.isNaN(Number(score))) {
      scores[sectionId] = Number(score);
    }
  });
...
/**
 * Builds CDP section rows for director view (web /director/assessments/result/cdp parity).
 */
export function buildCdpSectionRows(
  recommendationsPayload: unknown,
  assessmentSections: ApiAssessmentSection[] = [],
  answersPayload?: unknown,
): CdpSectionRow[] {
  const sectionMeta: SectionMeta[] = assessmentSections.map((s) => ({
    _id: s._id,
    title: s.title,
    name: s.title,
    layers: s.layers,
  ...
  const sentIds = collectSentSectionIds(recommendationsPayload);
  const scoresBySection = extractAnswersScoreBySection(answersPayload);

  let rows = parseRecommendationSectionsForPastorView(
    recommendationsPayload,
    sectionMeta,
  ).filter((row) => row.message.trim() !== '');

  if (sentIds.size > 0) {
    rows = rows.filter((row) => sentIds.has(row.sectionId));
  }

  const responseBody =
    recommendationsPayload && typeof recommendationsPayload === 'object'
      ? (recommendationsPayload as Record<string, unknown>)
      : {};
  const responseSections = Array.isArray(responseBody.sections)
    ? responseBody.sections
    : [];

  return rows.map((row, rowIndex) => {
    let sectionNumber = rowIndex + 1;
    const matchingIndex = responseSections.findIndex(
      (section: unknown) => {
        if (!section || typeof section !== 'object') return false;
        const s = section as Record<string, unknown>;
        return String(s.sectionId ?? s._id) === row.sectionId;
      },
    );
    if (matchingIndex >= 0) {
      sectionNumber = matchingIndex + 1;
    }

    const scoreFromAnswers = scoresBySection[row.sectionId];
    if (scoreFromAnswers != null) {
      return { ...row, sectionNumber, level: scoreFromAnswers };
    }

    const matchingSection = responseSections.find((section: unknown) => {
      if (!section || typeof section !== 'object') return false;
      const s = section as Record<string, unknown>;
      return String(s.sectionId ?? s._id) === row.sectionId;
    ...
    if (matchingSection && Array.isArray(matchingSection.recommendations)) {
      ...
      if (rec && typeof rec === 'object') {
        const level = (rec as { level?: number }).level;
        if (level != null) {
          return { ...row, sectionNumber, level };
        }
      }
    }
