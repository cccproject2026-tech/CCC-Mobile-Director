...
import { FlatList, Pressable, StyleSheet, Text, TouchableOpacity, View, RefreshControl } from "react-native";
...
export default function Mentors() {
    ...
    /* ------------------- STATES generated dynamically from API ------------------- */
    // const states = useMemo(() => {
    //     const list = mentors
    //         .map(m => m.profileInfo?.state)
    ...
    const filteredMentors = useMemo(() => {
        ...
        if (activeTab === "field_mentor") list = list.filter(m => m.role === "field_mentor");

        // if (selectedFilter && states.includes(selectedFilter))
        //     list = list.filter(m => m.profileInfo?.state === selectedFilter);
    ...
    const renderItem = ({ item }: { item: Mentor }) => (
        <TouchableOpacity onPress={() => router.push(`/(director)/(tabs)/mentors/${item.id}`)}>
            <MentorCard
                mentor={{
                    id: item.id,
            ...
            <View style={{ flex: 1 }}>
                ...
                <View style={{ flex: 1, paddingTop: 24 }}>
                    ...
                    <TabSwitcher
                        ...
                        activeTab={activeTab}
                        onChange={(key) => setActiveTab(key as 'all' | 'mentor' | 'field_mentor')}
                    />
                    ...
                    <View style={styles.filterContainer}>
                        ...
                        <Pressable style={styles.filterButton} onPress={() => setFilterModalVisible(true)}>
                            <Text style={styles.filterText}>{selectedFilter || "Select Filter"}</Text>
                ...
                <FilterModal
                    ...
                    onFilterSelect={f => { setSelectedFilter(f); setFilterModalVisible(false); }}
