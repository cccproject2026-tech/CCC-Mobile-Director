...
//   const { data: profileData, isLoading, isError } = useProfile(user?.id || "");
//   const updateProfile = useUpdateProfile(user?.email || "", user?.id || "");
...
//                 {profileData.user?.role.toUpperCase() || 'Director'}
//               </Text>
//             </View>
...
import { icons } from '@/constants';
import { Colors } from '@/constants/Colors';
import { useMentorMenteeProfile, useUpdateProfile, useUploadProfilePicture, useProfile } from '@/hooks/useProfile';
import { useAuthStore } from '@/stores/auth.store';
...
const CLEAN_CHURCH_TEMPLATE: ChurchInfo = {
  churchName: '', churchPhone: '', churchAddress: '',
  city: '', state: '', zipCode: '', country: '', churchWebsite: '',
};
...
export default function UnifiedProfileScreen() {
  ...
  // Context Logic: If paramId exists, we are viewing/editing someone else's profile.
  // Otherwise, we are viewing/editing our own profile.
  const targetUserId = paramId || authUser?.id || "";
  const isOwnProfile = !paramId;

  // Data Fetching: useMentorMenteeProfile handles fetching for any given ID
  const { data: profileData, isLoading, isError } = useMentorMenteeProfile(targetUserId);
  ...
  const handleEditPress = useCallback(() => {
    if (profileData?.user && profileData?.interest) {
      // MANUAL SYNC: Snapshot API data into local form state
      setFormData({
        firstName: profileData.user.firstName || '',
        lastName: profileData.user.lastName || '',
  ...
  if (isLoading) return <View style={styles.center}><Text>Loading profile...</Text></View>;
    ...
    <LinearGradient colors={['#176192', '#1D548D', '#264387']} style={styles.container}>
      ...
      >
        <View style={styles.profileHeader}>
          ...
          {!isEditing && (
            <>
              ...
              <Text style={styles.role}>{profileData?.interest?.title || profileData?.user?.role?.toUpperCase()}</Text>
            </>
          )}
        </View>
        ...
        <View style={styles.mainContentBox}>
          ...
          />
          <PersonalInfoSection isEditing={isEditing} profileData={profileData} formData={formData} onUpdateField={updateField} onPickImage={pickImage} profileImage={profileImage} showTitleDropdown={false} onTitleSelect={() => { }} onToggleTitleDropdown={() => { }} onUpdateChurch={updateChurch} onAddChurch={() => { }} onRemoveChurch={() => { }} />
          <ChurchInfoSection isEditing={isEditing} profileData={profileData} formData={formData} onUpdateChurch={updateChurch} onAddChurch={() => { }} onRemoveChurch={() => { }} showTitleDropdown={false} onTitleSelect={() => { }} onToggleTitleDropdown={() => { }} onPickImage={pickImage} profileImage={profileImage} onUpdateField={updateField} />
          <OtherInfoSection isEditing={isEditing} profileData={profileData} formData={formData} showTitleDropdown={false} onUpdateField={updateField} onTitleSelect={() => { }} onToggleTitleDropdown={() => { }} onUpdateChurch={updateChurch} onAddChurch={() => { }} onRemoveChurch={() => { }} onPickImage={pickImage} profileImage={profileImage} />
        </View>

        {isEditing && (
          <View style={styles.editActions}>
            <TouchableOpacity style={[styles.actionButtonSecondary, styles.cancelButton]} onPress={handleCancel}>
              <Text style={styles.cancelButtonText}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.actionButtonSecondary, styles.saveButton]}
      ...
      <SuccessModal visible={showSuccessModal} message="Profile Updated Successfully!" onClose={() => setShowSuccessModal(false)} />
    </LinearGradient>
  );
}
