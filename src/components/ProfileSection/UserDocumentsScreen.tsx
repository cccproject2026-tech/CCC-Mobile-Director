...
import { useDeleteDocument, useDocumentsByUserId, useUploadDocument, useUserProfile } from '@/hooks/useProfile';
...
export default function UserDocumentsScreen({ userId }: Props) {
    const { bottom } = useSafeAreaInsets();
    const { user: authUser } = useAuthStore();

    const targetUserId = userId || authUser?.id || '';
    const isOwnProfile = !!authUser?.id && targetUserId === authUser.id;

    const { data: profileUser } = useUserProfile(targetUserId);
    const { data: documents = [], isLoading, refetch } = useDocumentsByUserId(targetUserId);
    const uploadDocument = useUploadDocument(targetUserId);
    const deleteDocument = useDeleteDocument(targetUserId);
    ...
    return (
        <GradientBackground>
            <TopBar showUserName={isOwnProfile} showNotifications={isOwnProfile} />
